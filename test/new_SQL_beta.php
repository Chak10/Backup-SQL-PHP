...
class FILES extends FORMAT {
    ...
    protected static function std_file($str, $name_int, $name_ext = false) {
        $fname = $name_int;
        if ($name_ext != false) {
            $name_ext = pathinfo($name_ext, PATHINFO_FILENAME);
            if (!is_dir($name_ext))
    ...
    protected static function tar_compress($name_tar, $type = "gz", $index = 9) {
        switch ($type) {
            case 'gz':
                return file_put_contents($name_tar . '.gz', gzencode(file_get_contents($name_tar), $index, FORCE_GZIP)) && unlink($name_tar);
                break;
            case 'bz2':
    ...
    private function _save($str, $name_int, $name_ext, $dir_int = '') {
        if ($this->ext_c_supported == null) {
            if ($dir_int != '') {
                ...
                $name_ext = $dir_int . DIRECTORY_SEPARATOR . $name_ext;
            }
            return $this->std_file($str, $name_int, $name_ext);
        }
        if (in_array('tar', $this->ext_c_supported) && $this->compress == false) {
            if ($dir_int != '') {
        ...
        if (in_array('tar', $this->ext_c_supported) && $this->compress == "tar") {
            ...
            return $this->tar_file($str, $name_int, $name_ext . '.tar') && $this->tar_compress($name_ext . '.tar', "gz", 9);
        }
        if (!in_array('tar', $this->ext_c_supported) && $this->compress == "tar") {
            $this->compress = "zip";
        }
