<?php
	...
	class SQL_Backup extends FILES {
		...
		private function _save($str, $name_int, $name_ext, $dir_int = '') {
			if ($this->ext_c_supported == null) {
				if ($dir_int != '') {
					if(!is_dir($dir_int))mkdir($dir_int, 0764, true);
	...
	class TAR {
		...
		/**
			*	Code Not test only copied from http://php.net/manual/en/function.gzwrite.php
		...
		*/
		...
		/**
			...
			* @var $f :file resource provided by fopen
			* @var $phisfn: path to file
		*/
		
		function TarWriteContents($f,$phisfn) {   
			if (is_dir($phisfn)) return;
